...
$artifactsRoot = [IO.Path]::GetFullPath((Join-Path $repositoryRoot 'artifacts/release/0.2.0'))
...
try {
    ...
    Copy-Item -LiteralPath 'docs/releases/0.2.0/QUICK-START.md' -Destination $packageRoot
    Copy-Item -LiteralPath 'docs/releases/0.2.0/RELEASE-NOTES.md' -Destination $packageRoot
    Copy-Item -LiteralPath 'docs/releases/0.2.0/THIRD-PARTY-NOTICES.md' -Destination $packageRoot
    ...
    $manifest = [ordered]@{
        ...
        schemaVersion = '202608200002_ProjectSteering'
    ...
    $zipPath = Join-Path $artifactsRoot 'SASD-PIMS-0.2.0-win-x64.zip'
    Compress-Archive -Path (Join-Path $packageRoot '*') -DestinationPath $zipPath -CompressionLevel Optimal
    $hash = (Get-FileHash -LiteralPath $zipPath -Algorithm SHA256).Hash
    "$hash  $([IO.Path]::GetFileName($zipPath))" |
        Set-Content -LiteralPath (Join-Path $artifactsRoot 'SASD-PIMS-0.2.0-SHA256SUMS.txt') -Encoding ascii
