...
Push-Location $repo
try {
    DotNet @('clean','Sasd.Pims.slnx','-c','Release','--disable-build-servers','-m:1')
    DotNet @('restore','Sasd.Pims.slnx','--disable-build-servers','-m:1')
    DotNet @('build','Sasd.Pims.slnx','-c','Release','--no-restore','--disable-build-servers','-m:1','-p:UseSharedCompilation=false')
    if (-not $SkipTests) {
    ...
    if (-not $SkipVulnerabilityCheck) { DotNet @('package','list','--project','Sasd.Pims.slnx','--vulnerable','--include-transitive') }
    ...
    DotNet @('restore','src/Sasd.Pims.WinForms/Sasd.Pims.WinForms.csproj','-r','win-x64','--disable-build-servers','-m:1','-p:NuGetAudit=false')
    DotNet @('publish','src/Sasd.Pims.WinForms/Sasd.Pims.WinForms.csproj','-c','Release','-r','win-x64','--self-contained','true','-o',$publish,'--no-restore','--disable-build-servers','-m:1','-p:UseSharedCompilation=false')
