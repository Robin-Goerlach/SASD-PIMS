...
function Invoke-DotNet([string[]]$Arguments) { & dotnet @Arguments; if ($LASTEXITCODE -ne 0) { throw "dotnet failed: $($Arguments -join ' ')" } }
...
try {
    ...
    if (-not $SkipVulnerabilityCheck) { Invoke-DotNet -Arguments @('package','list','--project','Sasd.Pims.slnx','--vulnerable','--include-transitive') }
